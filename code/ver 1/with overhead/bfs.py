 ...
 # bfs.py
 #
 # Schedule and execute the task parts of the graph using the breadth-first
 # scheduler (BFS).
 #*************************************************************************
...
def schedule_tied(thr_num, num_tasks, num_threads, part_list):
...
def execute(task_type, num_tasks, num_threads, part_list, deadline, first_task_part, graphic_result):
	...
	# Initialize the scheduling time and the queue of ready parts #
	t = 0
	part_queue = []
	...
	for i in range(num_threads):
		if task_type == 'tied':
			thr = threading.Thread(target = schedule_tied, args = (i, num_tasks, num_threads, part_list, ))
		else:
